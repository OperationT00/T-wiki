...
const STAGES: Record<string, { start: number; weight: number; fallback: string }> = {
  ...
  "reading-media-info": { start: 50, weight: 3, fallback: "正在读取媒体信息" },
  "extracting-frames": { start: 53, weight: 14, fallback: "正在提取候选画面" },
  "filtering-frames": { start: 67, weight: 3, fallback: "正在筛选候选画面" },
  "visual-analysis": { start: 70, weight: 13, fallback: "正在进行视觉分析" },
  "building-markdown": { start: 83, weight: 5, fallback: "正在合成图文 Markdown" },
  "quality-check": { start: 88, weight: 6, fallback: "正在执行质量检查" },
