...
## 更新公告（0.1.7 · 2026-09-06）

本次更新增加固定质量与性能验收集，用同一套场景和阈值判断优化是否真的有效：

- **固定八类场景**：覆盖常规/增量 Ingest、直接/两跳 Query、批量 Parse、崩溃恢复和音视频质量。
- **稳定统计口径**：统一计算 P50、P95、均值、最小值和最大值，阈值可版本化审阅。
- **拒绝虚假通过**：每个场景至少 5 次；缺失指标、重复运行、非法比例或样本不足均判失败。
- **复用现有审计**：自动转换 Agent Trace 中的耗时、请求数、Token、重复读取、Evidence、精读页和图谱跳数。
- **语义质量显式评审**：候选遗漏、回答相关性、无依据结论和段落可读性必须依据固定答案评估，不以“流程成功”冒充质量通过。
- **本地隐私边界**：验收输入和报告只记录 ID、时间与数值，不保存正文、Prompt、回答、路径、Token 或服务地址。

固定验收不会在插件运行时上传遥测。场景定义见 `acceptance/suite-v1.json`，详细运行方法见 [固定质量与性能验收](docs/quality-acceptance.md)。0.1.6 的恢复中心和既有安全边界保持不变。
...
遇到异常退出或重启恢复问题时，可在 Obsidian 命令面板执行 **T-Wiki: 打开恢复中心**。
