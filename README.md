...
本次更新主要加强大批量解析、Ingest 审阅和 Wiki 写入的可靠性：

- **并发解析队列**：可以一次导入多份文件。解析任务使用有界 FIFO 队列，默认同时运行 3 个，可在设置中调整为 1–8 个；空闲槽位会自动执行下一个任务，排队任务支持取消。
- **Raw 发布恢复**：解析结果发布前会在现有 ParseAttempt 中记录候选 Revision。Raw 已发布但 Manifest 尚未提交时，插件重启后会校验 Raw、Hash 和 assets，校验通过则完成提交，异常时保留明确的可重试状态。
- **事务写入与恢复**：事务日志改为原子写入，并保留可校验的恢复信息；日志异常时可根据目标文件 Hash 判断已完成的写入，减少只能人工处理的情况。
- **待审阅 Ingest 恢复**：尚未确认的 Ingest 计划会持久化保存，Obsidian 或插件重启后仍可继续 Diff 审阅，不需要重新执行完整 Ingest。
- **Evidence 结论校验**：Evidence 不再只表示“读取过”，知识候选会携带受原文片段支持的结论声明，宿主会校验证据引用和对应文本。
- **增量 Wiki 索引**：单个 Wiki 页面新增、修改、删除或重命名时优先局部更新导航索引和 backlinks，避免任意变化都触发全库重建。
- **回滚存储去重**：事务和长期回滚记录改用内容寻址的压缩快照；相同旧内容只保存一次，降低 `.llm-wiki` 的长期空间占用。
- **音视频文字稿优化**：分片合并会结合重叠时间窗口消除重复内容，并保护数字、英文术语和否定表达；超长 ASR 片段会按自然句拆分并保持时间顺序。可选“约束式 Fast 模型”恢复标点和自然段，宿主会校验 Segment 顺序与字符守恒，失败时自动回退纯本地规则。

以上改动不改变 `raw/`、Wiki Schema、人工 Diff 审阅和事务 Apply 的既有安全边界。
